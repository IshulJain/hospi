...
def payment(request):
	...
	if request.method == 'POST':
		...
		try:
			...
			#techprofile = TechProfile.objects.get(email = post.get('identifier'))
		except:
			try:
		...
		deskteam = DeskTeam.objects.get(user = request.user)
		#offline = OffLineProfile(techProfile = techprofile, workshop = post['workshop'])	
		#offline.save(); 
		...
		if post['amount'] > 0:
			...
			transaction = Transaction(creditor = techprofile,amount = post['amount'], facility = facility, reciever = deskteam)
			transaction.save()
